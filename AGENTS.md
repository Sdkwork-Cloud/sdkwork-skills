...
Read `sdkwork.app.config.json` when the task touches Skills application behavior, runtime config, SDK wiring, release metadata, app-owned capabilities, packaging, or deployment. Skills marketplace persistence uses `ai_*` tables; kernel must consume `sdkwork-skills-contract` and skills APIs instead of local `a_agent_skill_package` persistence.
...
- `sdkwork.app.config.json`: Skills application identity, runtime, release, and capability metadata.
...
Read `../sdkwork-specs/CODE_STYLE_SPEC.md` and `../sdkwork-specs/NAMING_SPEC.md` before code changes. Keep skill persistence and CRUD in this repository; do not reintroduce skill tables in `sdkwork-kernel`. Generated SDK transport output is changed only through source contracts, generator inputs, or approved composed facades.
